...
# 兼容容器和本地开发环境，始终从项目根目录定位 dataset/food/user.csv 和 item.csv
from pathlib import Path
project_root = Path(__file__).resolve().parent.parent.parent
user_file_path = project_root / 'dataset' / 'food' / 'user.csv'
item_file_path = project_root / 'dataset' / 'food' / 'item.csv'
user_df = pd.read_csv(user_file_path)
item_df = pd.read_csv(item_file_path)
# 将 city_id、restaurant_id 和 food_id 列连接成新的 'id' 列
item_df['id'] = item_df['city_id'].astype(str) + '_' + item_df['restaurant_id'].astype(str) + '_' + item_df['food_id'].astype(str)

# 删除原有的 city_id, restaurant_id, food_id 列
item_df = item_df.drop(columns=['city_id', 'restaurant_id', 'food_id','food_name','image_name','spring','summer','autumn','winter', 'cuisine'])
...
# 随机采样 10% 的数据
rating_df = rating_df.sample(frac=0.1, random_state=42)  # 设置 random_state 以保证每次运行结果相同
...
# 可以将结果保存为新的 CSV 文件
output_file_path = project_root / 'dataset' / 'china-food' / 'china-food.inter'
