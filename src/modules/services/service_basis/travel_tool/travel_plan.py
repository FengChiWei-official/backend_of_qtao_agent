...
travelplan_desc = '''旅行规划：本接口为用户定制个性化的旅游方案，其囊括旅行途中每一天的交通、景点、餐馆和酒店。本接口输入格式为：{"用户需求": <用户需求>, "出发城市": <出发城市>, "目标城市": <目标城市>, "游玩天数": <游玩天数>, "人数": <人数>}，其中<用户需求>是结合对话历史对用户当前旅行规划需求的全面且准确的总结。'''
